...
def call_subprocess(cmd, show_stdout=True, cwd=None,
                    ...
                    extra_environ=None):
    if cmd[0] not in ("python", sys.executable):
        raise ValueError("Cannot execute commands other than 'python' on Pythonista (command: {})".format(cmd))
    elif not (cmd[1] == "-c" or cmd[1].endswith(".py")):
        raise ValueError("Can only execure Python files or runtime argument code using 'python' on Pythonista (command: {})".format(cmd))
    ...
    if show_stdout:
        ...
        # Need to use StringIO.StringIO instead of io.BytesIO, because
        # the former is more lenient about bytes/str/unicode differences.
        sys.stdout = fake_stdout = oldStringIO()
    """
    env = os.environ.copy()
    if extra_environ:
    ...
    try:
        if cmd[1] == "-c":
            ...
            with io.open(os.path.join(cwd, cmd[1]), "rb") as f:
                codestr = f.read()
        ...
        for key in set(os.environ) - set(old_env):
            del os.environ[key]
